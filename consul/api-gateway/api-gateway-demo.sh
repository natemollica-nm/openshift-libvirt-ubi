...
case "$APP" in
    ...
    default)
        ...
        "$OC_PATH" --context "${CONTEXT}" "$ACTION" -f api-gateway/api-gateway.yaml >/dev/null 2>&1 || {
            [ "$ACTION" = apply ] && \
        ...
        "$OC_PATH" --context "${CONTEXT}" "$ACTION" -f api-gateway/api-gw-route.yaml >/dev/null 2>&1 || {
            [ "$ACTION" = apply ] && \
        ...
        envsubst < api-gateway/ingress.yaml | "$OC_PATH" --context "${CONTEXT}" "$ACTION" -f - >/dev/null 2>&1 || {
            [ "$ACTION" = apply ] && \
