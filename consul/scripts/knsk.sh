...
# Variables
  ...
  FORCE=0      # Don't force deletion with kubeclt proxy by default
               # TODO: If in eks 1.17 or upper, try go get the reason of delayed ns deletion
...
# Search for resources in stuck namespaces
  ...
  if [ "x$NSS" == "x" ]; then
    ...
    for NS in $NSS; do
      ...
      RESS=$($K api-resources --verbs=list --namespaced -o name 2>/dev/null | grep -Ev ".*coreos\.com" | \
